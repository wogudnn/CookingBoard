		...
		<table class="grid">
			...
			<tr>
				...
				<c:forEach items="${articles}" var="article">
					<c:set var="number" value="${fn: split(article.cookingArticleId, '-')[2]}" ></c:set>
					<fmt:parseNumber var="number" type="number" value="${number}" ></fmt:parseNumber>
					<td>${number}</td>
					<td><a href="/CookingBoard/board/detail?articleId=${article.cookingArticleId}">${article.cookingArticleSubject}</a></td>
					<td>
					<c:choose>
